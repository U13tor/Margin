...
Bluetooth RSSI-based proximity detection — no GATT connection, no polling. When your paired device (active Bluetooth advertising devices such as watches and wristbands) moves ~5 meters away for 30+ seconds, Margin locks your workstation automatically. A 60-second cooldown prevents repeated locks when you return. **Invisible protection that never interrupts your flow.**
...
### 🦙 LlamaPet — Local LLM & GPU Telemetry Pet

A real-time telemetry pet companion designed for local LLM developers and edge inference enthusiasts:
- **Multi-Backend Support**: Deep integration with llama.cpp (native `/slots` and `/health`), Ollama, and OpenAI-compatible API endpoints;
- **NVIDIA GPU Hardware Telemetry**: VRAM usage, core utilization %, real-time power draw (W), and temperature (°C), powered by driver-level `nvml.dll` with graceful fallback to `nvidia-smi`;
- **Emotion-Driven Desktop Pet**: Dual modes with MiniPet (96×96) and Dock widget (260×50), featuring 7-state pixel llama animations reflecting inference workloads; ≤20px screen edge snap, Peek 3s auto-hide, Win32 click-through, and global summon shortcut `Ctrl+Alt+P`;
- **GeekHUD Dashboard**: High-density dashboard with 60-sample dual-trace VRAM/TPS Canvas real-time charts, per-SLOT status matrix grid, KV cache hit analysis, and one-click idle KV cache clearing.

> 📸 See detailed screenshots in [docs/02-install.md](docs/02-install.md) and [docs/10-llamapet.md](docs/10-llamapet.md).

---

## Privacy Commitment

- **Zero Network** — No heartbeat, no update checks, no crash reports. LlamaPet uses only the `localhost-http` permission (strictly locked to local loopback via code-level `LoopbackGuard`), verified with Wireshark
- **Zero Telemetry** — No usage data collected, all logs stay local
- **No Account Required** — No login, no tokens, works out of the box
- **End-to-End Encryption** — Sensitive fields (including model API keys) encrypted with AES-256-GCM, keys remain in the OS keyring
...
Windows MVP shipped. All four core plugins (Aura Locker / Screen Time / Rhythm & Health / LlamaPet) are fully functional, with a stable plugin architecture and permission model.
...
| [10-llamapet.md](docs/10-llamapet.md) | LlamaPet local LLM telemetry & desktop pet guide |
